...
@section("title", "Permohonan: $data->invoice")
@section("judul", " ")
    ...
    <div class="w-full overflow-x-auto">
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            ...
            <div class="border-t border-gray-200">
                <dl>
                    <form action="{{ route('permohonan.update', ['id'=>$data->invoice]) }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        @method('PUT')

                        <div class="bg-gray-200 px-4 py-5 sm:grid sm:grid-cols-2 sm:gap-4 sm:px-6">
                            <div class="ml-6">
                                    ...
                                    <ul role="list" class="border border-gray-200 rounded-md divide-y divide-gray-200">
                                        <li class="pl-3 pr-4 py-3 block items-center justify-between text-sm">
                                            <div class="mt-1 flex rounded-md shadow-sm mb-4">
                                                <select onchange="yesnoCheck(this);" name="status" id="status"
                                                    class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-max rounded-md sm:text-sm border-gray-300 px-5">
                                                    <option value="Batal" {{ $data->status == 'Batal' ? 'selected' : ''}}>Batal
                                                    </option>
                                                    <option value="Permohonan Diterima"
                                                        {{ $data->status == 'Permohonan Diterima' ? 'selected' : ''}}>Permohonan
                                                        Diterima</option>
                                                    <option value="Permohonan Bermasalah"
                                                        {{ $data->status == 'Permohonan Bermasalah' ? 'selected' : ''}}>Permohonan
                                                        Bermasalah</option>
                                                    @if ($data->jenispelayanan == "pnbp")
                                                    <option value="Menunggu Pembayaran"
                                                        {{ $data->status == 'Menunggu Pembayaran' ? 'selected' : ''}}>Menunggu
                                                        Pembayaran</option>
                                                    @endif
                                                ...
                                                <button type="submit"
                                                    class="ml-6 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                                    Update Status
                                                </button>
                                        ...
                                        <li class="pl-3 pr-4 py-3 flex items-center justify-between text-sm" id="ifYes" @if ($data->status !== "Selesai")
                                            ...
                                            <div class="flex text-sm text-gray-600 no-underline">
                                                <input id="data" name="data" type="file" class="not-sr-only"
                            ...
                            <div class="bg-gray-200 pl-6" id="buka" @if (($data->status !== "Diproses") || ($data->status !== "Selesai")) style="display: none" @endif>
                                ...
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                    <ul role="list" class="border border-gray-200 rounded-md divide-y divide-gray-200">
                                        <li class="pl-3 pr-4 py-3 flex items-center justify-between text-sm">
                                            <input value="{{$data->officer}}" type="text" name="officer" id="officer"
                                                class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 rounded-md sm:text-sm border-gray-300"
                                                placeholder="Isi inisial petugas, contoh: Satriyo">
                                        </li>
                                    </ul>
                                </dd>
                            </div>
                    ...
                    <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            ...
                            <ul role="list" class="border border-gray-200 rounded-md divide-y divide-gray-200">
                                @foreach ($produk as $p )
                                <li class="pl-3 pr-4 py-3 sm:block md:flex items-center justify-between text-sm">
                                    {{ $p->namalayanan }}
                            ...
                            <ul role="list" class="border border-gray-200 rounded-md divide-y divide-gray-200">
                                <li class="pl-3 pr-4 py-3 flex items-center justify-between text-sm">
                                    ...
                                    <div class="ml-4 flex-shrink-0">
                                        <a href="{{ Storage::url($data->suratpermohonan) }}"
                                ...
                                <li class="pl-3 pr-4 py-3 flex items-center justify-between text-sm">
                                    ...
                                    <div class="ml-4 flex-shrink-0">
                                        <a href="{{ Storage::url($data->scanktp) }}"
                                            class="font-medium text-indigo-600 hover:text-indigo-500">
                                            Download
                                        </a>
                                    </div>
                                ...
                                <li class="pl-3 pr-4 py-3 flex items-center justify-between text-sm">
                                    ...
                                    <div class="ml-4 flex-shrink-0">
                                        <a href="{{ Storage::url($data->suratpengantar) }}"
                                            class="font-medium text-indigo-600 hover:text-indigo-500">
                                            Download
                                        </a>
                                    </div>
                                ...
                                <li class="pl-3 pr-4 py-3 flex items-center justify-between text-sm">
                                    ...
                                    <div class="ml-4 flex-shrink-0">
                                        <a href="{{ Storage::url($data->suratpernyataan) }}"
                                            class="font-medium text-indigo-600 hover:text-indigo-500">
                                            Download
                                        </a>
                                    </div>
                                ...
                                <li class="pl-3 pr-4 py-3 flex items-center justify-between text-sm">
                                    ...
                                    <div class="ml-4 flex-shrink-0">
                                        <a href="{{ Storage::url($data->proposal) }}"
                                            class="font-medium text-indigo-600 hover:text-indigo-500">
                                            Download
                                        </a>
                                    </div>
...
<script>
    ...
    if (that.value == "Selesai") {
        ...
        document.getElementById("buka").style = " ";
    } else if (that.value == "Diproses") {
        document.getElementById("buka").style = " ";
    } else {
