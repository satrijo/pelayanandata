...
<div class="w-full overflow-hidden rounded-lg shadow-xs mb-10">
    <div class="w-full overflow-x-auto">
        ...
        <section class="max-w-full p-6 mx-auto bg-white rounded-md shadow-md dark:bg-gray-800">
            <form action="{{ route('faq.update', ['id' => $data->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="block gap-6 mt-4 sm:grid-cols-2">
                    <div class="block justify-center">
                        <input placeholder="Judul Pertanyaan" name="question" type="text" value="{{ $data->question }}"
                    ...
                    <div>
                        <textarea required name="answer" id="wysiwyg-editor"
                            class="ckeditor block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">{{ $data->answer }}</textarea>
                    </div>
                    <div class="block justify-center">
                        <button type="submit"
